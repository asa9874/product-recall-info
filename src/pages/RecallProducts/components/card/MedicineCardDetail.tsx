  ...
  return (
    <CardDetail onClose={onClose}>
      ...
      <div className="space-y-3">
        <p className="text-xs text-gray-500">등록일: {product.RECALL_COMMAND_DATE.split(" ")[0]}</p>
        <p className="text-xs text-gray-700 whitespace-pre-line">상세 내용: {product.RTRVL_RESN}</p>
      </div>

      <div className="mt-4 border-t pt-3">
        <p className="text-xs">게시글 번호: {product.ITEM_SEQ}</p>
      </div>
    </CardDetail>
  );
}
